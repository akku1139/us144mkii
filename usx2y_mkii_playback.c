...
static int tascam_playback_prepare(struct snd_pcm_substream *substream) {
  ...
  nominal_bytes_per_packet = nominal_frames_per_packet *
                             (substream->runtime->channels * BYTES_PER_SAMPLE);
...
void playback_urb_complete(struct urb *urb) {
  ...
  for (i = 0; i < urb->number_of_packets; i++) {
    ...
    bytes_for_packet =
        frames_for_packet * (runtime->channels * BYTES_PER_SAMPLE);
  ...
  if (total_bytes_for_urb > 0) {
    ...
    process_playback_routing_usx2y_mkii(tascam, dst_buf, dst_buf,
                                        frames_to_copy);
