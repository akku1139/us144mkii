...
static int tascam_capture_open(struct snd_pcm_substream *substream) {
  ...
  struct snd_pcm_hardware hw = tascam_pcm_hw;

  substream->runtime->hw = hw;
  tascam->capture_substream = substream;
  atomic_set(&tascam->capture_active, 0);

  switch (tascam->device_type) {
  case DEV_US144MKII:
    substream->runtime->hw.channels_min = 4;
    substream->runtime->hw.channels_max = 4;
    break;
  case DEV_US122MKII:
 ...
 */
static void decode_tascam_capture_block_2ch(const u8 *src_block,
                                            s32 *dst_block) {
  int frame, bit;

  memset(dst_block, 0, FRAMES_PER_DECODE_BLOCK * 2 * DECODED_SAMPLE_SIZE);

  for (frame = 0; frame < FRAMES_PER_DECODE_BLOCK; ++frame) {
    const u8 *p_src_frame_base = src_block + frame * 64;
    s32 *p_dst_frame = dst_block + frame * 2;
...
static void decode_tascam_capture_block_4ch(const u8 *src_block,
                                            s32 *dst_block) {
  int frame, bit;
...
void tascam_capture_work_handler(struct work_struct *work) {
  ...
  while (atomic_read(&tascam->capture_active)) {
    ...
    switch (tascam->device_type) {
    case DEV_US144MKII:
      decode_tascam_capture_block_4ch(raw_block, decoded_block);
      break;
    case DEV_US122MKII:
      decode_tascam_capture_block_2ch(raw_block, decoded_block);
      break;
    }

    process_capture_routing_usx2y_mkii(tascam, decoded_block, routed_block);
    ...
    if (atomic_read(&tascam->capture_active)) {
      ...
      for (f = 0; f < FRAMES_PER_DECODE_BLOCK; ++f) {
        ...
        s32 *routed_frame_start = routed_block + (f * runtime->channels);
        int c;
