...
void process_playback_routing_usx2y_mkii(struct tascam_card *tascam,
  ...
  if (tascam->device_type == DEV_US144MKII) {
    const u8 *src_12, *src_34;
    u8 *dst_line, *dst_digital;
    for (f = 0; f < frames; ++f) {
      src_12 = src_buffer + f * 12; // 4ch * 3 bytes
      src_34 = src_12 + 6;
      dst_line = dst_buffer + f * 12;
      dst_digital = dst_line + 6;

      if (tascam->line_out_source == 0)
        memcpy(dst_line, src_12, 6);
      else
        memcpy(dst_line, src_34, 6);

      if (tascam->digital_out_source == 0)
        memcpy(dst_digital, src_12, 6);
      else
        memcpy(dst_digital, src_34, 6);
    }
  } else { // DEV_US122MKII
    const u8 *src_frame;
    u8 *dst_frame;
    for (f = 0; f < frames; ++f) {
      src_frame = src_buffer + f * 6; // 2ch * 3 bytes
      dst_frame = dst_buffer + f * 6;
      // Channel Swap: src ch0 -> dst ch1, src ch1 -> dst ch0
      memcpy(dst_frame, src_frame + 3, 3);
      memcpy(dst_frame + 3, src_frame, 3);
    }
  }
}
...
void process_capture_routing_usx2y_mkii(struct tascam_card *tascam,
  ...
  if (tascam->device_type == DEV_US144MKII) {
    for (f = 0; f < FRAMES_PER_DECODE_BLOCK; f++) {
      src_frame = decoded_block + (f * 4);
      dst_frame = routed_block + (f * 4);

      // ch1/2 Source (Analog vs Digital)
      if (tascam->device_type == DEV_US144MKII) {
      ...
      // ch3/4 Source (Analog vs Digital)
    ...
    // Simple 2-channel passthrough
    for (f = 0; f < FRAMES_PER_DECODE_BLOCK; f++) {
      src_frame = decoded_block + (f * 2);
      dst_frame = routed_block + (f * 2);
      dst_frame[0] = src_frame[0];
      dst_frame[1] = src_frame[1];
    }
  }
...
int tascam_pcm_hw_params(struct snd_pcm_substream *substream,
  ...
  if (tascam->current_rate != rate) {
    err = usx2y_mkii_configure_device_for_rate(tascam, rate);
    if (err < 0) {
