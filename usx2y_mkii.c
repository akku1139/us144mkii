...
/*
 * ALSA Driver for TASCAM US-122MKII/US-144MKII Audio Interfaces
 */

#include "usx2y_mkii.h"

MODULE_AUTHOR("Šerif Rami <ramiserifpersia@gmail.com>");
MODULE_DESCRIPTION("ALSA Driver for TASCAM US-122MKII/144MKII");
MODULE_LICENSE("GPL");
 ...
 * for the TASCAM US-122MKII/144MKII is instantiated.
 *
...
int tascam_alloc_urbs(struct tascam_card *tascam) {
  ...
  max_packet_size = ((96000 / 8000) + 2) * BYTES_PER_SAMPLE;
...
static int tascam_resume(struct usb_interface *intf) {
  ...
  if (tascam->current_rate > 0)
    usx2y_mkii_configure_device_for_rate(tascam, tascam->current_rate);
...
static int tascam_probe(struct usb_interface *intf,
  ...
  case USB_PID_TASCAM_US144MKII:
    tascam->device_type = DEV_US144MKII;
    break;
  case USB_PID_TASCAM_US122MKII:
    tascam->device_type = DEV_US122MKII;
    break;
  default:
    dev_err(&dev->dev, "Unknown device type\n");
    goto free_card;
  }
  ...
  if (dev->descriptor.idProduct == USB_PID_TASCAM_US144MKII) {
    strscpy(card->shortname, "TASCAM US144MKII", sizeof(card->shortname));
  } else if (dev->descriptor.idProduct == USB_PID_TASCAM_US122MKII) {
    strscpy(card->shortname, "TASCAM US122MKII", sizeof(card->shortname));
  } else {
